    ...
    <div class="w-fit md:w-120 relative border p-4 shadow-xl bg-sky-50 dark:bg-sky-950">
      <div class="grid gap-2">
        ...
        <div class="flex items-center gap-2 ml-auto">
          <button
            class="bg-black dark:bg-white hover:opacity-80 text-sm text-white dark:text-black px-4 py-1 cursor-pointer shadow-sm transition duration-200"
          >
            {{ actionMessage() }}
          </button>
        </div>
...
export class CustomToast {
  protected readonly title = input<string>('Custom Toast');
  ...
  protected readonly actionMessage = input<string>('Action');
}
